...
function renderCategories() {
  ...
  const NON_FOOD_CATS = new Set(['Cuidado facial y corporal', 'Limpieza y hogar', 'Maquillaje',
    'Cuidado del cabello', 'Cuidado personal', 'Fitoterapia y parafarmacia']);
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:12px">
      ${sorted.map(({cat, withMacros, total}) => `
        <div onclick="selectCategory('${cat.replace(/'/g,"\\'")}')"
          ...
          <div style="display:flex;justify-content:center;margin-bottom:8px">${getCategoryIcon(cat, 32)}</div>
...
function renderCategoryProducts(cat) {
  ...
  content.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px">
      ...
      <span style="display:flex">${getCategoryIcon(cat, 24)}</span>
      <div>
    ...
    <div id="subcat-tabs" style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:16px"></div>
    <div id="subcat-sections"></div>`;

  loadFullCatalog(catalog => {
    const allProducts = catalog.filter(p => (p.category || 'Otros') === cat);
    const countEl = document.getElementById('cat-count');
    const withoutMacrosCount = allProducts.length - withMacros.length;
    if (countEl) countEl.textContent = `${withMacros.length} con macros · ${withoutMacrosCount} sin macros`;

    const combined = allProducts.map(p => withMacrosIds.has(p.id) ? withMacros.find(f=>f.id===p.id) : p);
    ...
    const subcats = Object.keys(bySubcat).sort((a,b) => bySubcat[b].length - bySubcat[a].length);

    const tabsEl = document.getElementById('subcat-tabs');
    if (tabsEl) {
      tabsEl.innerHTML = subcats.map(sub => `
        <button onclick="scrollToSubcat('${sub.replace(/'/g,"\\'")}')"
          style="background:var(--bg);border:1px solid var(--border);border-radius:20px;padding:6px 14px;font-size:12px;font-weight:500;cursor:pointer;color:var(--text2);white-space:nowrap">
          ${sub} <span style="color:var(--text3)">(${bySubcat[sub].length})</span>
        </button>`).join('');
    }

    const sectionsEl = document.getElementById('subcat-sections');
    if (sectionsEl) {
      sectionsEl.innerHTML = subcats.map(sub => {
        const items = bySubcat[sub];
        const withM = items.filter(p => withMacrosIds.has(p.id));
        const withoutM = items.filter(p => !withMacrosIds.has(p.id));
        return `
          <div id="subcat-${sub.replace(/[^a-zA-Z0-9]/g,'_')}" style="margin-bottom:28px;scroll-margin-top:16px">
            <div style="font-size:15px;font-weight:700;margin-bottom:12px;padding-bottom:8px;border-bottom:2px solid var(--border)">${sub}</div>
            <div class="foods-grid">${renderFoodCards(withM, withoutM)}</div>
          </div>`;
      }).join('');
    }
  });
...
function scrollToSubcat(sub) {
  const el = document.getElementById(`subcat-${sub.replace(/[^a-zA-Z0-9]/g,'_')}`);
...
function renderFoodCards(withMacros, withoutMacros) {
  ...
  // Separar los "sin macros" en: no-alimentos (desbloqueados, solo precio) vs alimentos reales sin macros (bloqueados)
  const nonFoodItems = withoutMacros.filter(p => p.is_food === false);
  const unlockableItems = withoutMacros.filter(p => p.is_food !== false);

  const nonFoodCards = nonFoodItems.map(p => `
    <div class="fc" style="cursor:pointer" onclick="openProductModal('${p.id}', false)">
      ${p.thumbnail ? `<img src="${p.thumbnail}" style="width:100%;height:110px;object-fit:contain;border-radius:var(--rs);margin-bottom:10px;background:#f9fafb;" loading="lazy">` : ''}
      <div class="fc-name">${p.name}</div>
      <div class="fc-brand">${p.brand}${p.category?` · <span style="color:var(--text3)">${p.category}</span>`:''}</div>
      <div class="fc-price" style="margin-top:12px;padding-top:10px;border-top:1px solid var(--border)">
        <span style="font-size:16px;font-weight:700;color:var(--green)">${p.price}€</span>
        <span style="color:var(--text3)">/${p.unit}</span>
      </div>
    </div>`).join('');

  const noMacroCards = unlockableItems.map(p => `
  ...
  if (!macroCards && !noMacroCards && !nonFoodCards) return `<div style="grid-column:1/-1;text-align:center;padding:40px;color:var(--text3)">Sin productos</div>`;
  return macroCards + nonFoodCards + noMacroCards;
}

function openProductModal(foodId, hasMacros) {
  const f = hasMacros ? FOODS.find(x => x.id === foodId) : (CATALOG||[]).find(x => x.id === foodId);
  if (!f) return;
  const isNonFood = f.is_food === false;
  ...
  // Si es no-alimento, macrosHTML se queda vacio — no mostramos nada de macros

  let actionsHTML;
  if (hasMacros) {
    actionsHTML = `<button class="btn btn-primary" style="flex:1" onclick="addToPantryModal('${f.id}');document.getElementById('product-modal').classList.remove('open')">+ Añadir a despensa</button>
               <button class="btn" onclick="editFoodMacros('${f.id}')">✏️ Editar</button>`;
  } else if (isNonFood) {
    actionsHTML = ''; // sin botones de macros/despensa para no-alimentos
  } else {
    actionsHTML = `<button class="btn btn-primary" style="flex:1" onclick="openUnlockModal('${f.id}')">🔓 Añadir macros</button>`;
  }
    ...
    <div class="modal" style="width:520px;max-height:85vh;overflow-y:auto">
      ...
      <div style="padding:16px">
        ...
        <div style="margin-bottom:16px">
          <img id="product-main-img" src="${f.thumbnail}" onclick="openImageLightbox(this.src)" style="width:100%;height:200px;object-fit:contain;border-radius:var(--r);background:#f9fafb;cursor:zoom-in">
          ${(f.photos && f.photos.length > 1) ? `
            <div style="display:flex;gap:6px;margin-top:8px;overflow-x:auto;padding-bottom:4px">
              ${f.photos.map((url, i) => `
                <img src="${url}" onclick="document.getElementById('product-main-img').src='${url}'"
                  style="width:50px;height:50px;object-fit:contain;border-radius:6px;background:#f9fafb;border:2px solid ${i===0?'var(--green)':'var(--border)'};cursor:pointer;flex-shrink:0">
              `).join('')}
            </div>` : ''}
        </div>` : ''}
...
// ── Seccion Cuidado personal y Hogar (productos no alimenticios) ──────────
let nonFoodCategory = null;
let NON_FOOD_CATALOG = [];

function loadNonFoodCatalog(onReady) {
  if (NON_FOOD_CATALOG.length > 0) { onReady(NON_FOOD_CATALOG); return; }
  loadFullCatalog(catalog => {
    NON_FOOD_CATALOG = catalog.filter(p => p.is_food === false);
    onReady(NON_FOOD_CATALOG);
  });
}

function renderNonFood(el) {
  el.innerHTML = `
    <div class="search-bar" style="margin-bottom:20px">
      <input id="nonfood-search" placeholder="Buscar en drogueria y cuidado personal..." oninput="filterNonFood()"/>
    </div>
    <div id="nonfood-content">
      <div style="text-align:center;padding:40px;color:var(--text3)">Cargando...</div>
    </div>`;

  loadNonFoodCatalog(() => {
    if (nonFoodCategory) renderNonFoodCategoryProducts(nonFoodCategory);
    else renderNonFoodCategories();
  });
}

function renderNonFoodCategories() {
  const content = document.getElementById('nonfood-content');
  if (!content) return;

  const cats = {};
  NON_FOOD_CATALOG.forEach(p => {
    const cat = p.category || 'Otros';
  ...
  content.innerHTML = `
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:12px">
      ${sorted.map(([cat, count]) => `
        <div onclick="selectNonFoodCategory('${cat.replace(/'/g,"\\'")}')"
          style="background:var(--surface);border:1px solid var(--border);border-radius:var(--r);padding:16px 12px;text-align:center;cursor:pointer;transition:all 0.15s"
          onmouseover="this.style.borderColor='var(--green)';this.style.background='var(--green-light)'"
          onmouseout="this.style.borderColor='var(--border)';this.style.background='var(--surface)'">
          <div style="display:flex;justify-content:center;margin-bottom:8px">${getCategoryIcon(cat, 32)}</div>
          <div style="font-size:12px;font-weight:600;color:var(--text);line-height:1.3">${cat}</div>
          <div style="font-size:11px;color:var(--text3);margin-top:4px">${count} productos</div>
        </div>`).join('')}
    </div>`;
}

function selectNonFoodCategory(cat) {
  nonFoodCategory = cat;
  renderNonFoodCategoryProducts(cat);
}

function backToNonFoodCategories() {
  nonFoodCategory = null;
  renderNonFoodCategories();
}

function renderNonFoodCategoryProducts(cat) {
  const content = document.getElementById('nonfood-content');
  if (!content) return;
  const products = NON_FOOD_CATALOG.filter(p => (p.category || 'Otros') === cat);

  content.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px">
      <button onclick="backToNonFoodCategories()" style="background:none;border:1px solid var(--border);border-radius:var(--rs);padding:6px 12px;font-size:13px;cursor:pointer">← Categorias</button>
      <span style="display:flex">${getCategoryIcon(cat, 20)}</span>
      <div>
        <div style="font-size:15px;font-weight:600">${cat}</div>
        <div style="font-size:12px;color:var(--text3)">${products.length} productos</div>
      </div>
    </div>
    <div class="foods-grid">${renderNonFoodCards(products)}</div>`;
}

function renderNonFoodCards(products) {
  if (!products.length) return `<div style="grid-column:1/-1;text-align:center;padding:40px;color:var(--text3)">Sin productos</div>`;
  return products.map(p => `
    <div class="fc" style="cursor:pointer" onclick="openNonFoodModal('${p.id}')">
      ${p.thumbnail ? `<img src="${p.thumbnail}" style="width:100%;height:130px;object-fit:contain;border-radius:var(--rs);margin-bottom:10px;background:#f9fafb;" loading="lazy">` : ''}
      <div class="fc-name">${p.name}</div>
      <div class="fc-brand">${p.brand||''}</div>
      <div class="fc-price" style="margin-top:10px;padding-top:10px;border-top:1px solid var(--border)">
        <span style="font-size:16px;font-weight:700;color:var(--green)">${p.price}€</span>
        <span style="color:var(--text3)">/${p.unit}</span>
      </div>
    </div>`).join('');
...
function filterNonFood() {
  const q = document.getElementById('nonfood-search')?.value || '';
  const content = document.getElementById('nonfood-content');
  if (!content) return;
  if (q.trim().length < 2) {
    if (nonFoodCategory) renderNonFoodCategoryProducts(nonFoodCategory);
  ...
  const filtered = NON_FOOD_CATALOG.filter(p =>
    p.name.toLowerCase().includes(q.toLowerCase()) ||
    (p.brand||'').toLowerCase().includes(q.toLowerCase())
  );
  content.innerHTML = `
    <div style="font-size:13px;color:var(--text2);margin-bottom:12px">${filtered.length} resultados para "<strong>${q}</strong>"</div>
    <div class="foods-grid">${renderNonFoodCards(filtered)}</div>`;
}

function openNonFoodModal(productId) {
  const p = NON_FOOD_CATALOG.find(x => x.id === productId);
  if (!p) return;
  ...
  modal.innerHTML = `
    <div class="modal" style="width:480px;max-height:80vh;overflow-y:auto">
      ...
      <div style="padding:16px">
        ${p.thumbnail ? `<img src="${p.thumbnail}" style="width:100%;height:180px;object-fit:contain;border-radius:var(--r);background:#f9fafb;margin-bottom:16px">` : ''}
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px">
          <div>
            <div style="font-size:16px;font-weight:700">${p.name}</div>
            <div style="font-size:13px;color:var(--text2)">${p.brand||''} · ${p.category||''}</div>
          </div>
          <div style="text-align:right">
            <div style="font-size:18px;font-weight:700;color:var(--green)">${p.price}€</div>
            <div style="font-size:11px;color:var(--text3)">por ${p.unit_size||1}${p.unit}</div>
          </div>
        </div>
        ${p.description ? `<div style="font-size:13px;color:var(--text2);margin-bottom:12px">${p.description}</div>` : ''}
        ${p.usage_instructions ? `<div style="margin-bottom:12px"><div style="font-size:11px;font-weight:600;color:var(--text3);text-transform:uppercase;margin-bottom:4px">Modo de uso</div><div style="font-size:12px;color:var(--text2)">${p.usage_instructions}</div></div>` : ''}
        ${p.share_url ? `<a href="${p.share_url}" target="_blank" class="btn" style="width:100%;text-align:center;display:block;text-decoration:none;box-sizing:border-box">Ver en Mercadona ↗</a>` : ''}
      </div>
    </div>`;
  modal.classList.add('open');
}
...
function openImageLightbox(url) {
  ...
  if (!modal) {
    ...
    modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.85);z-index:200;display:flex;align-items:center;justify-content:center;cursor:zoom-out;padding:20px';
    modal.onclick = () => modal.remove();
    document.body.appendChild(modal);
  }
  modal.innerHTML = `<img src="${url}" style="max-width:95%;max-height:95vh;object-fit:contain;border-radius:8px">`;
}