...
const PAGES_DEF = [
  ...
  {id:'planner',   label:'Planificador',     emoji:'📅'},
  {id:'recetas',   label:'Mis comidas',      emoji:'🍽️'},
  {id:'dietas', label:'Mis dietas', emoji:'🗂️'},
  {id:'alimentos', label:'Alimentos',        emoji:'🥦'},
  {id:'despensa',  label:'Despensa',         emoji:'📦'},
  {id:'compra',    label:'Lista de compra',  emoji:'🛒'},
  {id:'ajustes',   label:'Mi perfil',        emoji:'👤'},
  {id:'coach', label:'Entrenador IA', emoji:'💪'},
  {id:'nonfood',   label:'Cuidado y Hogar',  emoji:'🧴'},
];

function buildNav() {
  document.getElementById('nav').innerHTML = PAGES_DEF.map(p =>
    `<button class="nav-item${p.id===currentPage?' active':''}" onclick="goTo('${p.id}')">
      <span class="nav-icon">${twemojiImg(p.emoji, 18)}</span> ${p.label}
    </button>`
  ).join('');
}
...
const PAGE_TITLES = {
  ...
  nonfood:   'Cuidado personal y productos del hogar',
  coach: 'Tu entrenador personal',
  dietas:   'Mis dietas guardadas',
};
...
const PAGE_ACTIONS = {
  ...
  dietas: '<button class="btn btn-primary btn-sm" onclick="saveDiet();setTimeout(()=>renderDietas(document.getElementById(\'page-dietas\')),200)">💾 Guardar semana actual</button>',
};
...
function render(id) {
  ...
  else if (id==='nonfood')   renderNonFood(el);
  else if (id==='dietas') renderDietas(el);
  else if (id==='coach') renderCoach(el);
}
...
function saveDiet() {
  if (window.editingDietId) {
    ...
    const diet = diets.find(d => d.id === window.editingDietId);
    if (diet) {
      diet.week = JSON.parse(JSON.stringify(week));
      save('saved_diets', diets);
      showToast(`"${diet.name}" actualizada`);
