...
function renderPlannerWeek(el) {
  ...
  function getMealLabel(m) { return MEAL_LABELS[m] || '🍴'; }
  ...
  const dayCols = DAYS.map((d, di) => {
    ...
    return `
      <div style="flex:1;min-width:130px">
        <div style="background:${dateInfo.isToday?'#e6f7ee':DAY_COLORS[di]};border:${dateInfo.isToday?'2px solid #00A650':'1px solid '+DAY_BORDER[di]};border-radius:8px 8px 0 0;padding:10px 10px 8px;border-bottom:3px solid ${DAY_BORDER[di]}">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:4px">
            <div>
              <div style="display:flex;align-items:center;gap:5px">
                <div style="font-size:13px;font-weight:800;color:#111">${d}</div>
                ${dateInfo.isToday ? `<span style="background:#00A650;color:white;font-size:8px;font-weight:700;padding:1px 6px;border-radius:6px">HOY</span>` : ''}
              </div>
              <div style="font-size:10px;color:#6b7280">${dateInfo.dateStr}</div>
            </div>
  ...
  el.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
      <div class="view-toggle">
        ...
        <button class="btn btn-sm" onclick="openAddMealTypeModal()">+ Tipo de comida</button>
      </div>
...
function openAddMealTypeModal() {
  let modal = document.getElementById('meal-type-modal');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'meal-type-modal';
    modal.className = 'modal-bg';
    modal.onclick = e => { if (e.target === modal) modal.classList.remove('open'); };
    document.body.appendChild(modal);
  }

  const SUGGESTIONS = ['Pre-entreno', 'Post-entreno', 'Merienda', 'Cena tardía', 'Media mañana', 'Antes de dormir'];
    ...
    <div class="modal" style="--modal-w:400px">
      ...
      <div style="padding:16px">
        <div class="form-group">
          ...
          <input class="form-input" id="new-meal-type-name" placeholder="Ej: Batido post-gym">
        </div>
        <button class="btn btn-primary" style="width:100%;margin-bottom:16px" onclick="confirmAddMealType()">Crear</button>

        <div style="font-size:12px;font-weight:600;color:var(--text2);margin-bottom:8px">O elige una sugerencia</div>
        <div style="display:flex;flex-wrap:wrap;gap:6px">
          ${SUGGESTIONS.filter(s => !customs.includes(s)).map(s => `
            <button class="btn btn-sm" onclick="addCustomMealType('${s}');document.getElementById('meal-type-modal').classList.remove('open');renderPlanner(document.getElementById('page-planner'))">${s}</button>
          `).join('')}
        </div>

        ${customs.length ? `
          <div style="font-size:12px;font-weight:600;color:var(--text2);margin:16px 0 8px">Tus tipos personalizados</div>
          <div style="display:flex;flex-wrap:wrap;gap:6px">
            ${customs.map(c => `
              <span style="background:var(--bg);border-radius:20px;padding:4px 10px;font-size:12px;display:flex;align-items:center;gap:6px">
                ${c} <button onclick="removeCustomMealType('${c}');renderPlanner(document.getElementById('page-planner'));document.getElementById('meal-type-modal').classList.remove('open')" style="background:none;border:none;cursor:pointer;color:var(--text3)">✕</button>
              </span>`).join('')}
          </div>` : ''}
      </div>
    </div>`;
...
function confirmAddMealType() {
  const name = document.getElementById('new-meal-type-name').value.trim();
  if (!name) return;
  const ok = addCustomMealType(name);
  if (!ok) { showToast('Ese tipo ya existe'); return; }
  document.getElementById('meal-type-modal').classList.remove('open');
  renderPlanner(document.getElementById('page-planner'));
}